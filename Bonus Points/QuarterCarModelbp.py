...
class SpringK2(qtw.QGraphicsItem):
    def __init__(self, startX, startY, endX, endY, pen=None, length=50):
        super().__init__()
        self.startX = startX
        self.startY = startY
        self.endX = endX
    ...
    def boundingRect(self):
        return qtc.QRectF(self.startX, self.startY, self.endX - self.startX, self.endY - self.startY)

    def paint(self, painter, option, widget=None):
        painter.setPen(self.pen)
        painter.drawLine(self.startX, self.startY, self.endX, self.endY)

class SpringK1(qtw.QGraphicsItem):
    def __init__(self, startX, startY, endX, endY, pen=None, length=50):
        super().__init__()
        self.startX = startX
        self.startY = startY
...
class LegendItem(qtw.QGraphicsItem):
    ...
    def boundingRect(self):
        return qtc.QRectF(0, 0, 100, 20)  # Adjust width and height as needed

    def paint(self, painter, option, widget=None):
        painter.setPen(self.pen)
        painter.drawText(self.boundingRect(), qtc.Qt.AlignLeft, self.text)
...
class CarModel():
    ...
    def __init__(self):
        ...
        self.m2 = 40  # mass of wheel in kg
        self.c1 = 1000  # damping coefficient in N*s/m
        self.k1 = 20000  # spring constant of suspension in N/m
        self.k2 = 40000  # spring constant of tire in N/m
        self.v = 60  # velocity of car in kph

        self.mink1 = 1000  # If I jack up my car and release the load on the spring, it extends about 3 inches
        self.maxk1 = 50000  # What would be a good value for a soft spring vs. a stiff spring?
        self.mink2 = 5000  # Same question for the shock absorber.
        self.maxk2 = 80000
        self.accel = None
        self.accelMax = 20  # Assuming a maximum acceleration of 20 m/s^2
        self.accelLim = 2 * 9.81  # Assuming a maximum acceleration of ~2g
        self.SSE = 0.0
...
class CarView():
    ...
    def buildScene(self):
        ...
        self.SpringK2 = SpringK2(0, 100, 0, 60, pen=self.penSpringK2, length=50)
        self.scene.addItem(self.SpringK2)
        self.SpringK1 = SpringK1(-25, 40, -25, -54, pen=self.penSpringK1, length=50)
        self.scene.addItem(self.SpringK1)
        self.Dashpot = Dashpot(25, 40, 25, -54, pen=self.penDashpot, length=50)
        self.scene.addItem(self.Dashpot)

        # Add legend items
        self.SpringK1Legend = LegendItem("Spring (k1)", self.penSpringK1)
        self.SpringK2Legend = LegendItem("Spring (k2)", self.penSpringK2)
        self.DashpotLegend = LegendItem("Dashpot (c1)", self.penDashpot)

        # Add legend items to the scene
        self.scene.addItem(self.SpringK1Legend)
        self.scene.addItem(self.SpringK2Legend)
        self.scene.addItem(self.DashpotLegend)

        # Position legend items
        legend_height = 20  # Adjust height as needed
        legend_spacing = 5  # Adjust spacing as needed
        self.SpringK1Legend.setPos(150, -180)  # Adjust position as needed
        self.SpringK2Legend.setPos(150, -180 + legend_height + legend_spacing)
        self.DashpotLegend.setPos(150, -180 + 2 * (legend_height + legend_spacing))
    ...
    def setupPensAndBrushes(self):
        ...
        self.penSpringK2 = qtg.QPen(qtg.QColor("blue"))
        self.penSpringK2.setWidth(2)
        self.penSpringK1 = qtg.QPen(qtg.QColor("purple"))
        self.penSpringK1.setWidth(2)
        self.penDashpot = qtg.QPen(qtg.QColor("green"))
        self.penDashpot.setWidth(2)
...
class CarController():
    ...
    def ode_system(self, X, t):
        ...
        x1ddot = (self.model.k1 / self.model.m1) * (y - x1) - (self.model.c1 / self.model.m1) * (x1dot - x2dot)
        x2ddot = (self.model.k2 / self.model.m2) * (x1 - x2)
    ...
    def calculate(self, doCalc=True):
        ...
        self.model.mink1 = 0.75 * (self.model.m1 / 450)  # Assuming minimum static compression of 0.75 inches
        self.model.maxk1 = 1.5 * (self.model.m1 / 450)  # Assuming maximum static compression of 1.5 inches
        self.model.mink2 = 0.25 * (self.model.m1 / 450)  # Assuming minimum static compression of 0.25 inches
        self.model.maxk2 = 0.5 * (self.model.m1 / 450)  # Assuming maximum static compression of 0.5 inches
    ...
    def OptimizeSuspension(self):
        ...
        # Step 2:
        # Making an initial guess for k1, c1, and k2
        x0 = np.array([self.model.k1, self.model.c1, self.model.k2])

        # Step 3:
        # Optimizing the suspension using the Nelder-Mead method to minimize the SSE function
        answer = minimize(self.SSE, x0, method='Nelder-Mead')

        # Updating the model and view with the optimized values
...
def main():
    ...
    le_m2 = qtw.QLineEdit()
    le_k2 = qtw.QLineEdit()
    le_ang = qtw.QLineEdit()
    le_tmax = qtw.QLineEdit()
    chk_IncludeAccel = qtw.QCheckBox()

    input_widgets = [le_m1, le_v, le_k1, le_c1, le_m2, le_k2, le_ang, le_tmax, chk_IncludeAccel]

    # Create display_widgets
    gv_Schematic = qtw.QGraphicsView()
    chk_LogX = qtw.QCheckBox()
    chk_LogY = qtw.QCheckBox()
    chk_LogAccel = qtw.QCheckBox()
    chk_ShowAccel = qtw.QCheckBox()
    lbl_MaxMinInfo = qtw.QLabel()
    layout_horizontal_main = qtw.QHBoxLayout()

    display_widgets = [gv_Schematic, chk_LogX, chk_LogY, chk_LogAccel, chk_ShowAccel, lbl_MaxMinInfo, layout_horizontal_main]

    # Pass the arguments to CarController
    QCM = CarController((input_widgets, display_widgets))
    QCM.doCalc()
